...
import { ActSchema } from "@/lib/schemas";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await stagehand.act(parsed.data.instruction);
    const description = result.message || result.actionDescription || `Performed: ${parsed.data.instruction}`;
